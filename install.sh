...
clear
echo -e "${CYAN}Installing Termux Tx Cmd Tool...${NC}"
sleep 1

# Required packages
echo -e "${GREEN}[+] Installing dependencies (curl, jq)...${NC}"
pkg update -y && pkg install curl jq -y

# Create working directory
mkdir -p $HOME/termux-tx-cmd
cd $HOME/termux-tx-cmd

# Download the main script
echo -e "${GREEN}[+] Downloading IP Tracker Script...${NC}"
cat > tx-ip-tracker.sh << 'EOF'
#!/bin/bash
...
# Make it executable
chmod +x tx-ip-tracker.sh

# Done
echo -e "${CYAN}\n[✓] Installed successfully!"
echo -e "${GREEN}Run the tool using: ${YELLOW}cd ~/termux-tx-cmd && ./tx-ip-tracker.sh${NC}"
